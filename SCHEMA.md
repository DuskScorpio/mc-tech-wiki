...
- **Footnote formatting (Obsidian + OKF):** citations use `[^id]` inline references and `[^id]: <url>` definitions at the bottom (OKF v0.2 §5.1). CRITICAL: (1) the def block must be **preceded by a blank line**. (2) refs are SINGLE-bracket `[^id]` — NEVER `[[^id]]`. (3) footnote **ids must be URL/path-safe** (only `[a-z0-9_-]`): no spaces, no dots, no `:`. A spaced/malformed id (e.g. `[^ Sapling]` or `[^minecraft.wiki Tutorial:Tree_farming]`) is unresolvable and Obsidian renders it as a broken `#` link with back-arrows. Every `[^id]:` def must have a URL or `/concepts/` path. Lint must flag: `[[^id]]` double-bracket; id with space/illegal char; def not preceded by blank line; def target not `http(s)://` or `/concepts/`.
