...
- **Footnote formatting (Obsidian + OKF):** citations use `[^id]` inline references and `[^id]: <url>` definitions at the bottom (OKF v0.2 §5.1). CRITICAL: (1) the def block must be **preceded by a blank line** — Obsidian fails to register defs attached to preceding content, breaking ALL footnotes in the file. (2) refs are SINGLE-bracket `[^id]` — NEVER `[[^id]]` (double), which Obsidian parses as a wiki-link and creates a stray note for it. (3) every `[^id]:` def must have a URL or `/concepts/` path. Lint must flag: `[^id]` written as `[[^id]]`; def not preceded by blank line; def target not `http(s)://` or `/concepts/`.
