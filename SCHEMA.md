...
- **Claim-to-source traceability (enforced, MED):** every factual claim in a concept body MUST trace to a cited raw source — no inference, generalization, or "plausible" statement written as fact without a source. Specifically: (1) any sentence making a concrete factual assertion (mechanic, number, behavior, "all/most/every" claim) MUST carry an inline `[^id]` footnote to a `sources[]` entry; (2) a footnote id used inline MUST resolve to a `[^id]:` def whose target raw file actually supports the claim (do not cite a source for a statement it does not make — cross-applying a detail from one source to a different topic is a violation, e.g. attributing a dustless-wiring leaf-check mechanism to general growth detection); (3) when a source says a thing is *optional* / *not required* / *version-dependent*, the concept must preserve that qualifier — do not upgrade a qualified claim into an absolute one. Lint must flag: a body sentence ending in a factual assertion with no inline `[^id]`; an inline `[^id]` whose `sources[]` entry / raw file does not contain the claimed term (best-effort keyword check). The agent MUST read the cited raw file and confirm the claim line-before committing — structural lint cannot catch wrong attributions, only the author can.
