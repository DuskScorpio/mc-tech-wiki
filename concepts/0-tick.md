...
A 0-tick action moves functionality from inter-tick into intra-tick timing — the main way to speed up a tree farm. See [[piston-action-timing]] for the mechanism (rising edge at one depth, deeper falling edge in the same BE).^[raw/articles/gtmc-tree-farm-high-speed.md]
...
- **Redstone redirection (dustless):** the only dustless method that outputs a *zero-delay rising edge* — see [[dustless-wiring]].^[raw/articles/gtmc-tree-farm-dustless-wiring.md]
