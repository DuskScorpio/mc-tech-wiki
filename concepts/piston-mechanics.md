...
Piston = base + head; states retracted/extended. A **headless piston** (base only) believes its head still exists in front of it.^[raw/articles/gtmc-pistons.md]
...
- **shouldExtend** checks the piston's own 6 directions (excluding facing), then the block BELOW, then the block ABOVE's 6 directions. This is the origin of **QC** and why BUDs only detect NC updates.^[raw/articles/gtmc-pistons.md]
...
- b36 arrival/removal order = reverse of the moved/broken lists.^[raw/articles/gtmc-pistons.md]

## b36 (moving_piston) & placement
- Normal: b36 gains 0.5 progress/tick in the TE phase; from 0gt AT it's fully placed at **3gt AT** → the "3-gt piston delay".^[raw/articles/gtmc-pistons.md]
