...
- Height **6 to 9** blocks. (Minecraft Wiki describes dark oak as "typically 6–8" — the code range is 6–9.)[^bilibili-dark-oak-growth]
...
- If the 2×2 trunk's **NW corner already has a log**, that layer's other logs don't grow — but side branches are unaffected. Same rule applies on a bent layer's NW corner.[^bilibili-dark-oak-growth]
...
- Lower bound = one above root; upper bound = one below final trunk top.[^bilibili-dark-oak-growth]
...
- If the trunk bends, the leaf center shifts the same direction by the same amount — **fixed in 1.21.4** (MC-237375): newer versions ignore the bend for centering.[^bilibili-dark-oak-growth]
...
[^bilibili-dark-oak-growth]: raw/articles/bilibili-dark-oak-growth.md
[^gtmc-tree-farm-multi-species]: raw/articles/gtmc-tree-farm-multi-species.md
[^mcwiki-dark-oak]: raw/articles/mcwiki-dark-oak.md
